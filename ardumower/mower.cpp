...
Mower::Mower() {
  ...
  sonarToFrontDist           = 20;        // ultrasonic sensor distance to front mower in cm
  ...
  DistPeriOutRev = 20; // reverse distance when reach the perimeter in cm
  DistPeriObstacleRev = 15; // reverse distance when hit obstacle while tracking in cm
  DistPeriOutForw = 60; // distance to accell
  DistPeriOutStop = 10; //slowing distance after crossover the wire
  DistPeriObstacleForw = 40; //distance while arc circle in peri obstacle avoid
  ...
  yawCibleNeg = -90;
  DistBetweenLane = 38;
  maxLenghtByLane = 20;  // distance to run in bylane before simulate a wire detection
  ...
  odometryTicksPerRevolution = 690;   // encoder ticks per one full resolution
  odometryTicksPerCm = 8.6;  // encoder ticks per cm
  odometryWheelBaseCm = 35;    // wheel-to-wheel distance (cm)
  ...
  mowPatternDurationMax = 180; //in minutes
